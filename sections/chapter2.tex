...
互信息 $I(x;y)$ 表示由事件 $y$ 发生所减少的关于 $x$ 的\textbf{不确定性}。等价地说，它是事件 $y$ 给出的关于 $x$ 的\textbf{信息量}。
\end{keypoint}
...
前面 $I(x;y)$ 是事件层面的互信息，取决于具体取到的值，可能为正也可能为负。在系统分析和性能评估中，我们真正需要的是平均意义上的指标——即对所有可能的 $(x,y)$ 取值做加权平均，这就是平均互信息 $I(X;Y)$。
...
$I(X;Y)$ 表示从 $Y$ 平均获得关于 $X$ 的信息量，可以理解为 $X$ 与 $Y$ 之间「共享」的信息。它永远是\textbf{非负}的——多知道一个变量，平均而言不会让你更困惑。
\end{keypoint}
...
因此 $I(X;Y)=H(Y)-H(Y\mid X) = (1-p)(1-\log(1-p))$。\\[2pt]
...
\begin{itemize}
  ...
  \item $H(X \mid Y)=0$（$Y$ 完全决定 $X$）与 $I(X;Y)=0$（$X$ 与 $Y$ 独立）；
...
互信息 & $I(x;y)=\log\frac{p(x\mid y)}{p(x)}=I(x)-I(x\mid y)$ & (4) \\
自信息关系 & $I(x,y)=I(x)+I(y\mid x)$ & (5) \\
互信息对称性 & $I(x;y)=I(y;x)=\log\frac{p(x,y)}{p(x)p(y)}$ & (6) \\
\midrule
信息熵 & $H(X)=-\sum_x p(x)\log p(x)$ & (7) \\
二元熵 & $h(p)=-p\log p-(1-p)\log(1-p)$ & (8) \\
联合熵 & $H(X,Y)=-\sum_{x,y}p(x,y)\log p(x,y)$ & (9) \\
条件熵 & $H(X\mid Y)=\sum_y p(y)H(X\mid Y=y)$ & (10) \\
\midrule
熵的关系 & $H(X,Y)=H(X)+H(Y\mid X)$ & (11) \\
 & $H(X\mid Y)\le H(X)$ & (12) \\
 & $0\le H(X)\le\log|\mathcal{X}|$ & (13) \\
\midrule
相对熵 & $\KL(P\|Q)=\sum_x P(x)\log\frac{P(x)}{Q(x)}\ge 0$ & (14) \\
\midrule
平均互信息 & $I(X;Y)=\sum_{x,y}p(x,y)\log\frac{p(x,y)}{p(x)p(y)}$ & (15) \\
 & $I(X;Y)=H(X)-H(X\mid Y)=H(Y)-H(Y\mid X)$ & (16) \\
 & $\;\,\qquad=H(X)+H(Y)-H(X,Y)$ & (17) \\
 & $I(X;Y)\ge 0$，$I(X;Y)\le\min\{H(X),H(Y)\}$ & (18) \\
条件互信息 & $I(X;Y\mid Z)=H(X\mid Z)-H(X\mid Y,Z)$ & (19) \\
\midrule
熵的链则 & $H(X_1^n)=\sum_{i=1}^{n}H(X_i\mid X_1^{i-1})$ & (20) \\
互信息链则 & $I(X_1^n;Y)=\sum_{i=1}^{n}I(X_i;Y\mid X_1^{i-1})$ & (21) \\
\midrule
数据处理不等式 & $X\!\to\!Y\!\to\!Z\;\Rightarrow\;I(X;Z)\le I(X;Y)$ & (22) \\
\bottomrule
\end{tabularx}
}
