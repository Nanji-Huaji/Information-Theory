...
本章主要回答三个问题：信息论研究什么、通信系统怎么抽象、香农信息论为什么重要。读这一章时，重点抓住信息的层次、通信系统模型，以及三大定理的含义。
\end{remarkBox}
...
三大编码定理概括了信息论的三个核心边界：\textbf{香农第一定理}说明，如果编码后的信源序列信息传输速率不小于\textbf{信源熵}，则可实现无失真编码；反之，不存在无失真编码。\textbf{香农第二定理}说明，如果信息传输速率小于\textbf{信道容量}，则当编码序列足够长时，总可找到一种编码使传输差错任意小；反之，不存在使差错任意小的编码。\textbf{香农第三定理}说明，对任何失真测度 $D \ge 0$，只要码字足够长，总可找到一种编码，使得当信息传输率满足 $R(D)$ 时，平均失真不超过 $D$。$R(D)$ 称为\textbf{信息率失真函数}。
...
\midrule
性能指标   & \textcolor{Red}{有效性}（码率）、\textcolor{Red}{可靠性}（误码率）、\textcolor{Red}{安全性}（加密） \\
\midrule
\textcolor{Red}{香农第一定理} & \textcolor{Red}{信息传输速率 $\ge$ 信源熵 $\Rightarrow$ 可实现无失真编码} \\
\midrule
\textcolor{Red}{香农第二定理} & \textcolor{Red}{信息传输速率 $<$ 信道容量 $\Rightarrow$ 可使差错任意小} \\
\midrule
\textcolor{Red}{香农第三定理} & \textcolor{Red}{信息传输率 $\ge R(D)$ 时，平均失真 $\le D$；$R(D)$ 为率失真函数} \\
\midrule
信息三个层次 & 语法信息（形式）$\to$ 语义信息（含义）$\to$ 语用信息（效用） \\
\midrule
信源分类   & \textcolor{Red}{离散 / 连续；无记忆 / 有记忆} \\
\midrule
信道分类   & \textcolor{Red}{狭义 / 广义；无噪声 / 有噪声；AWGN 为最常见模型} \\
\bottomrule
\end{tabularx}
}
